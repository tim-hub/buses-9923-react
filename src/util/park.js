...
class Park {
  ...
  place(x, y, facing, index = this.getCountOfBuses()) {
    if (this.getCountOfBuses() >= width * length) {
      ...
      logger.log(
        "alert",
        `The park is already full, cannot be parked here - (${x}, ${y})`
      );
    } else if (x >= width || x < 0 || y >= length || y < 0) {
      // outside of park
      logger.log(
        "alert",
        `You cannot park to (${x}, ${y}) - outside of car park, you will get a ticket!!!`
      );
    } else if (
      this.buses.find(bus => {
        return bus.x === x && bus.y === y;
      })
    ) {
      // already a bus parking here
      logger.log(
        "alert",
        `There is a bus already here - (${x}, ${y}), cannot be parked here.`
      );
    } else {
